...
car$manufacturer[car$manufacturer %in% c("Maruti","Honda","Toyota","Mitsubishi","Nissan","Lexus","Isuzu", "Hyudai", "Mahindra", "Tata", "Datsun", "Daewoo", "Kia", "Force", "Ashok")] <- "Asia"

car$manufacturer[car$manufacturer %in% c("Skoda","Mercedes-Benz","Audi","Volkswagen","BMW", "Renault", "Land", "MG", "Volvo", "Fiat", "Opel")] <- "Europe"

car$manufacturer[car$manufacturer %in% c("Ford","Chevrolet","Jeep")] <- "US"
...
```
```{r}
full.model = lm(selling_price ~ .- torque , data = car)
...
```{r}
full.model = lm(selling_price ~ .- torque , data = car)
step.model_BIC_for = stepwise(full.model,
     direction = c("both", "backward", "forward"))
#step.model_BIC_for = stepBIC(full.model, direction = "forward", trace = FALSE)
#step.model_BIC_back = stepBIC(full.model, direction = "backward", trace = FALSE)
#step.model_BIC_both= stepBIC(full.model, direction = "both", trace = FALSE)

summary(step.model_BIC_for)
#summary(step.model_BIC_back)
#summary(step.model_BIC_both)
```
```{r}
...
# huber
robust_huber.lm <- rlm(selling_price ~ . - torque, data = car, psi = psi.huber)
...
# bisqure
robust_bisquare.lm <- rlm(selling_price ~ . - torque, data = car, psi = psi.bisquare)
summary(robust_bisquare.lm)
plot(robust_bisquare.lm)
(weights <- robust_bisquare.lm$w)
plot(weights, main = "bisquare: Weights v.s. the Observation Number")
...
train.lm <- lm(selling_price ~ .  - torque - manufacturer, data = train_data)
summary(train.lm)
...
#RMSPE = RMSE(preds, ytest)
#MAPE = MAE(preds, ytest)
