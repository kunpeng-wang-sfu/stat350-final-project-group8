...
sub_1 = cbind(Mileage,Engine,Max_power)
sub_2 = sub_1<-sub_1[,-c(2,4,6)]

car_temp <- cbind(Car_details_v3, petrol, diesel, dealer, manual, first_owner, second_owner)
car <- cbind(car_temp, sub_2)


colnames(car)[which(names(car) == "1")] <- "Mileage"
colnames(car)[which(names(car) == "2")] <- "Engine"
colnames(car)[which(names(car) == "3")] <- "Max_power"

car<-subset(car, select = -c(mileage,engine,max_power))
